...
from util import ProtestDataset, modified_EffnetB1, modified_resnet50, AverageMeter, Lighting, modified_EffnetB2, modified_EffnetB3


# for indexing output of the model
protest_idx = torch.LongTensor([0])
violence_idx = torch.LongTensor([1])
visattr_idx = torch.LongTensor(range(2,12))
best_loss = float("inf")
...
def calculate_loss(output, target, criterions, weights = [1, 10, 5]):
    ...
    if N_protest == 0:
        ...
        targets = [None]
        # protest target
        targets[0] = target['protest'].float().unsqueeze(1)
    ...
    # print('not_protest_mask: ', not_protest_mask)
    outputs = [None] * 4
    # print(outputs)
    # protest output
    outputs[0] = output.index_select(1, protest_idx)
    ...
    targets[0] = targets[0].unsqueeze(1)
    targets[1] = targets[1].unsqueeze(1)
...
def save_checkpoint(state, is_best, filename='checkpoint.pth.tar', filename2='model_best.pth.tar'):
    """Save checkpoints"""
...
def main():
    ...
    if args.check_point:
        print("Loading checkpoint from: ",args.model_finetune)
        checkpoint = torch.load(args.model_finetune)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        }, is_best, filename=args.checkpoint_dir, filename2=args.model_dir)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--check_point',
                        default='', type=str,
                        help='whether to load checkpoint or not (default: none)')
    ...
    parser.add_argument("--checkpoint_dir",
                        type=str,
                        default = '',
                        help = "directory path to save checkpoint file",
                        )
    ...
    parser.add_argument("--model_finetune",
                        type=str,
                        default = 'model_best_ResNet50.pth.tar',
                        help = "name of the model for finetune",
                        )
